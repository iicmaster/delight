  ...
  <script>
    $(function() {
      $('#inputLocation').on('change', function() {
        var option = parseInt($('option:selected', this).attr('price'));
        var total = parseInt($('#total').html());
        $('#shipping-fee').html(option);
        $('#grand-total').html(option + total);
      });
...
@section('content')
  <div>
    <form id="form" class="form-horizontal" action="/cart/checkout">
      <h1>Cart</h1>
        ...
        <tbody>
          ...
          @forelse ($cart as $product)
            <tr>
              ...
              <td class="right">{{ Helper::add_comma($product->price) }}</td>
      ...
      <div class="control-group">
        ...
        <div class="controls">
          <input type="text" placeholder="Name" id="inputName" name="name" required>
        </div>
      </div>
      <div class="control-group">
        <label for="inputTel" class="control-label">Tel</label>
        <div class="controls">
          <input type="text" placeholder="Tel" id="inputTel">
        </div>
      </div>
      <div class="control-group">
        <label for="inputLocation" class="control-label">Location</label>
        <div class="controls">
          <select id="inputLocation" name="location" required>
      ...
      <div class="control-group">
        ...
        <div class="controls">
          <textarea type="text" placeholder="Address" id="inputAddress" required></textarea>
        </div>
      </div>
      <hr>
      <h1>Total</h1>
      <table class="table table-striped table-bordered">
        <tbody>
          <tr>
            ...
            <td class="right"><span id="total">{{ Helper::add_comma($total) }}</span></td>
          </tr>
            <td class="right">Shipping fee</td>
            <td class="right"><span id="shipping-fee">0</span></td>
        </tbody>
        <tfoot>
          </tr>
            <td class="right">Grand Total</td>
            <td class="right"><span id="grand-total">0</span></td>
          </tr>
        </tfoot>
      </table>
      <hr>
